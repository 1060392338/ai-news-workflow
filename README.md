...
新注册的头条号有 **14天新手观察期**，前7天流量受限，需要按以下策略养号：

### 阶段策略

| 阶段 | 天数 | 每天发布 | 重点 |
|------|------|----------|------|
| 初始化 | 第1-3天 | 1篇 | 垂直AI内容，固定话题标签 |
| 稳定输出 | 第4-7天 | 1-2篇 | 丰富内容类型，引导评论互动 |
| 拓展期 | 第8-14天 | 2-3篇 | 尝试不同风格，关注同行 |
| 正常运营 | 14天后 | 1-3篇 | 根据数据优化选题 |

### 使用养号 Pipeline

修改 `config.yaml` 中租户的 `pipeline_type: nurturing` 即可开启养号模式：

```yaml
tenants:
  - id: toutiao_ai_a
    pipeline_type: nurturing  # 养号模式：每天限发1篇
    ...
```

养号模式（`pipelines/nurturing.py`）自动：
- **每日限发1篇**（记录在 `publish_record.json`）
- **自动勾选话题标签**（根据内容匹配 #人工智能 #AI #大模型 等）
- **跳过已达上限的发布**

### 话题标签（自动匹配）

发布时根据文章标题和正文自动匹配话题标签，发到 `extra.interest_tags` 字段。匹配词库在 `services/workflow_graph.py` 的 `_TOPIC_TAGS` 中维护。
